    ...
    private void matchProduct(String alias, String productGroup, double coefficient, String single, String productCategory, Product product)
    {
        ...
        for (String match : matches)
        {
            ...
            if (StringUtils.startsWithIgnoreCase(type, match.trim()))
            {
                ...
                int finalPriceRounded   = (int) (90*(Math.ceil(Math.abs((finalPrice)/90))));
                product.setFinalPrice(finalPriceRounded);

                /// ОКРУГЛЕНИЕ ВВЕРХ!
                int bonus           = finalPriceRounded * 3 / 100;
                int bonusRounded    = (int) (10*(Math.ceil(Math.abs((bonus)/10))));
                if (bonusRounded == 0) product.setBonus(10);
                product.setBonus(bonusRounded);
                ...
                log.info("Price with %: "   + finalPrice);
                log.info("Price rounded: "  + finalPriceRounded);
